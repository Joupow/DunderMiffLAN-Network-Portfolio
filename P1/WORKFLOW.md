...
→ Plan d'adressage complet → [`IPAM.md`](https://claude.ai/IPAM.md)
...
- [Niveaux & équipements](#niveaux--%C3%A9quipements)

**2. Exécution**

- [Étape 1 — Déploiement physique](#%C3%A9tape-1--d%C3%A9ploiement-physique)
- [Étape 2 — VLANs de base](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#%C3%A9tape-2--vlans-de-base)
- [Étape 3 — Trunks 802.1Q](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#%C3%A9tape-3--trunks-8021q)
- [Étape 4 — Ports d'accès + durcissement](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#%C3%A9tape-4--ports-dacc%C3%A8s--durcissement-de-bordure)
- [Étape 4b — Config IP des PC](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#%C3%A9tape-4b--config-ip-des-pc-gui)
- [Étape 5 — SVIs + routage inter-VLAN](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#%C3%A9tape-5--svis--routage-inter-vlan-core)
- [Étape 6 — VLAN de management 99](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#%C3%A9tape-6--vlan-de-management-99)
- [Étape 7 — STP : Rapid PVST+](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#%C3%A9tape-7--stp--rapid-pvst-root-sur-la-distribution)

**3. Preuves & clôtures**

- [Validation de bout en bout (gate)](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#validation-de-bout-en-bout-gate-final)
- [Dépannage (incidents de session)](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#d%C3%A9pannage-incidents-de-session)
- [Registre d'erreurs & dette technique](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#registre-derreurs--dette-technique-%C3%A9tat-final)
- [Annexe — Captures de preuve](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#annexe--captures-de-preuve)
...
![Networ-overview-P1](https://claude.ai/assets/network-overview/NO_P1.png)
...
## <a id="étapes-de-configuration-vue-densemble"></a>Étapes de configuration
...
```
[1]  Déploiement physique + câblage (Gigabit core/inter-Dist ; uplinks access 100M sur ACC Fa0/1-2)
[2]  VLANs de base (10,20,30,99,998,999) — sur CHAQUE switch
[3]  Trunks 802.1Q (natif 999, VLAN 1 exclu, allowed 10,20,30,99,999)
[4]  Ports d'accès (data, voix, isolation 998) + durcissement bordure (PortFast + BPDU Guard, ports PC UNIQUEMENT)
[4b] Config IP des PC (statique + passerelle par VLAN)
[5]  SVIs + ip routing sur le Core (passerelles temporaires)
[6]  VLAN de management 99 (SVI + ip default-gateway sur chaque switch L2)
[7]  STP : rapid-pvst sur CHAQUE switch + root primary/secondary sur la Distribution
[8]  Validation (intra/inter-VLAN + management + failover)
```
...
**Validation :** identifier les ports physiquement up _avant_ de configurer.
...
> 📷 **[P-00](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-00)** topologie as-built · **[P-01](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-01)** CORE `show interfaces status` (Gi1/0/1-2 trunk).
...
> 📷 **[P-02](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-02)** `show vlan brief` — CORE canonique (jumeaux par switch pour les 7).
...
> 📷 **[P-03](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-03)** `show interfaces trunk` — DIST-SW1 + DIST-SW2, `Fa0/1-4` en trunk, natif 999.
...
> ⚠️ **PortFast + BPDU Guard sur les ports vers hôtes UNIQUEMENT (`Fa0/3`, `Fa0/4`) — jamais sur les uplinks `Fa0/1-2`.** BPDU Guard err-disable un port dès qu'il reçoit une BPDU ; un uplink en échange en continu, donc le garder tue le lien à sa première BPDU normale (**incident #2**). La protection de boucle sur les uplinks redondants est le travail de STP (`Altn BLK`), pas de BPDU Guard.
...
> 📷 **[P-04](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-04)** ACC `show interfaces status` — `Fa0/3`=10, `Fa0/4`=20, inutilisés disabled/998.

---

### <a id="étape-4b--config-ip-des-pc-gui"></a>Étape 4b — Config IP des PC (GUI)

**Intention :** donner à chaque PC une IP statique, un masque `/24` et une passerelle par défaut cohérents avec son VLAN. Valeurs de build **transitoires** (remplacées par DHCP en P2) — plan de référence dans [`IPAM.md`](https://claude.ai/IPAM.md).
...
> ⚠️ La passerelle va dans le champ **Default Gateway**, jamais dans le champ DNS — confusion classique du « l'inter-VLAN ne marche que dans un sens ».
...
> ⚠️ Dépendance : `ip routing` **doit** précéder les blocs `interface vlan`. Sans lui, le 3650 reste L2 et les SVIs ne routent jamais. ⚠️ **Dette de conception :** ces adresses `.1` sont provisoires — elles migrent sur la Distribution en VIP HSRP en P2. **Ne jamais recréer un SVI `.1` sur le Core après P2** (conflit de VIP).
...
> 📷 **[P-05](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-05)** PC1 ping inter-VLAN `.20.10` + intra-VLAN cross-switch `.10.12`.
...
> ⚠️ En P1, les 3560 sont capables de L3 mais utilisés en **L2 pur** (le routage inter-VLAN vit uniquement sur le Core). Un switch L2 pur ne consulte pas de table de routage : il a besoin d'`ip default-gateway` pour que son propre trafic de management sorte du VLAN 99, comme les 2960. Note : `ip default-gateway` est ignoré si `ip routing` est actif — donc ne **pas** activer `ip routing` sur la Distribution en P1.
...
> 📷 **[P-06](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-06)** DIST-SW2 `show ip interface brief` (Vlan99 `192.168.99.12` up/up) · **[P-07](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-07)** PC1 ping `192.168.99.1`.
...
> ⚠️ **Régler le mode sur CHAQUE switch — pas seulement les roots.** Rapid PVST+ négocie son handshake rapide (proposal/agreement) par lien ; un seul switch laissé en `pvst` classique sur ce lien fait retomber tout le segment sur les timers lents 802.1D. Mélanger les modes coûte silencieusement la reconvergence sous-seconde.
...
> ⚠️ **Le Core n'est PAS root, intentionnellement.** Ne pas lancer `root primary` sur le Core. Poser `root primary/secondary` sur la Distribution force des priorités (24576 / 28672) qui battent le 32768 par défaut du Core.
...
> ⚠️ **Portée du root.** Fixé de façon déterministe sur la Distribution pour 10, 20, 30, 99 **et 999**. Le VLAN 999 ne porte aucun hôte (natif trou noir), mais son root est épinglé sur DIST-SW1 par hygiène — aucun switch d'accès ne doit être root de quoi que ce soit. **998** n'est pas dans la liste allowed des trunks : aucune BPDU 998 ne traverse les liens, chaque switch est root isolé de sa propre instance 998 locale.
...
> 📷 **[P-08](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-08)** DIST-SW1 root `{10,30,999}` (rapid-pvst) · **[P-09](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-09)** DIST-SW2 root `{20,99}` (rapid-pvst) · **[P-10](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-10)** les 7 switches en `rapid-pvst mode`.
...
|---|---|---|---|---|
|🗺️ Topologie|Uplinks Core|`show interfaces status`|Gi1/0/1-2 `connected trunk`|[P-01](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-01)|
|🔌 Commutation|VLANs|`show vlan brief`|10/20/30/99/998/999 actifs sur les 7 switches|[P-02](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-02)|
|🔌 Commutation|Trunks|`show interfaces trunk`|DIST `Fa0/1-4` + Gig en trunk, natif 999, liste allowed complète|[P-03](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-03)|
|🔌 Commutation|Bordure access|`show interfaces status`|`Fa0/3`=10, `Fa0/4`=20, inutilisés disabled/998|[P-04](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-04)|
|🌳 STP|Root STP|`show spanning-tree summary`|DIST1 root `{10,30,999}`, DIST2 root `{20,99}`|[P-08](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-08), [P-09](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-09)|
|🌳 STP|Mode STP|`show spanning-tree summary`|`rapid-pvst mode` sur les 7|[P-10](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-10)|
|📦 Connectivité|Inter/intra-VLAN|PC1 `ping .20.10` + `.10.12`|réponse (1er paquet ARP toléré)|[P-05](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-05)|
|📦 Connectivité|Management|DIST-SW2 `show ip int brief` + PC `ping .99.1`|Vlan99 `.99.12` up/up ; passerelle répond|[P-06](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-06), [P-07](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-07)|
|🔁 Haute dispo|**Failover**|couper l'uplink `Root FWD` d'un ACC, re-ping la passerelle|uplink bloqué (`Altn BLK`→`Root FWD`) promu, trafic repris ; `no shutdown` restaure|[P-11](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-11)→[P-14](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-14)|

> ℹ️ Un unique `Request timed out` sur le **premier** paquet d'un nouveau flux (puis 0 %) est de la résolution ARP + convergence STP — pas une faute. Une perte persistante sur un flux établi est un vrai symptôme. ⚠️ **Timing du failover :** rapid-pvst confirmé ([P-10](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-10)) → reconvergence sous-seconde attendue ; le ping de **mesure directe** sous rapid-pvst reste à capturer.

---

## <a id="dépannage-incidents-de-session"></a>Dépannage (incidents de session)

> Historique de build — incidents rencontrés et comment chacun a été attrapé. Ce ne sont **pas** des dettes : chacun a été corrigé dans la même session. L'état final de chaque point est en §5.

|#|Symptôme|Cause|Diagnostic|Correctif|
|---|---|---|---|---|
|1|Downlink DIST `connected 1` (pas `trunk`) ; `%CDP-4-NATIVE_VLAN_MISMATCH` face à l'Access|`Fa0/1-4` de la Distribution laissés en access VLAN 1 (bloc downlink de l'étape 3 non appliqué)|`show interfaces status` + `show interfaces trunk`|Appliquer la config trunk aux DIST `Fa0/1-4` (natif 999, allowed) · avant : [P-TS1](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-ts1) · après : [P-03](https://claude.ai/chat/2bd001af-59d8-4865-9fb2-b9e7e0d3bdb2#p-03)|
...
> ⚠️ **Numérotation stable inter-parties.** Ces numéros sont des identifiants cités par d'autres docs (ex. `README_P3` → « P1 #8 Port Security »). Ne pas les renuméroter.
...
![Capture P1-13](https://claude.ai/assets/captures/P1/Capture_P1_13.png)

![Capture P1-14](https://claude.ai/assets/captures/P1/Capture_P1_14.png)

**<a id="p-04"></a> [P-04] · Bordure access + durcissement** — ACC-SW4 `show interfaces status` (`Fa0/3`=10, `Fa0/4`=20, inutilisés disabled/998)

![Capture P1-11](https://claude.ai/assets/captures/P1/Capture_P1_11.png)
...
![Capture P1-07](https://claude.ai/assets/captures/P1/Capture_P1_07.png)

**<a id="p-06"></a> [P-06] · SVI de management** — DIST-SW2 `show ip interface brief`, Vlan99 `192.168.99.12` up/up

![Capture P1-05](https://claude.ai/assets/captures/P1/Capture_P1_05.png)

**<a id="p-07"></a> [P-07] · Joignabilité management** — PC1 `ping 192.168.99.1`

![Capture P1-06](https://claude.ai/assets/captures/P1/Capture_P1_06.png)
...
![Capture P1-04](https://claude.ai/assets/captures/P1/Capture_P1_04.png)

**<a id="p-12"></a> [P-12] · Failover — action** — ACC-SW1 `interface Fa0/1` → `shutdown`

![Capture P1-03](https://claude.ai/assets/captures/P1/Capture_P1_03.png)
...
![Capture P1-02](https://claude.ai/assets/captures/P1/Capture_P1_02.png)

**<a id="p-14"></a> [P-14] · Failover — après** — ACC-SW1 STP v10 : `Fa0/2` promu `Root FWD` (coût 23 via DIST-SW2)

![Capture P1-01](https://claude.ai/assets/captures/P1/Capture_P1_01.png)

**<a id="p-ts1"></a> [P-TS1] · Incident #1 · Mismatch natif (AVANT correction)** — DIST-SW2 `%CDP-4-NATIVE_VLAN_MISMATCH` `Fa0/3-4` ![Capture P1-09](https://claude.ai/assets/captures/P1/Capture_P1_09.png)

> Compagnons périmés (état _avant_ correction, dépannage uniquement — jamais en validation) : DIST-SW2 `Capture_P1_14` · DIST-SW1 `Capture_P1_15` (`Fa0/1-4 connected 1`).

---

⬆️ [README de la partie](https://claude.ai/chat/README.md) · [Vue d'ensemble du projet](https://claude.ai/README.md) — Suivant : [WORKFLOW Partie 2](https://claude.ai/P2/WORKFLOW.md) (migration des SVIs sur la Distribution, HSRP, OSPF P2P, DHCP + relais).