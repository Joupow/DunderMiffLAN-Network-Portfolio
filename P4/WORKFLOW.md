...
- Sur chaque **Spine** — `show ip ospf neighbor` → **4** voisins, tous `FULL/ -`. [P-01]
- Sur chaque **Border Leaf** — **3** voisins (2 Spines + Core, une fois l'étape 4 faite), tous `FULL/ -` ; `show ip route 172.16.2.0` = ECMP via les deux Spines. [P-03]
- Sur chaque **Leaf** — **2** voisins, tous `FULL/ -`. [P-04]
...
**Validation :**
- `show ip ospf neighbor` (Core) → **5** voisins : `2.2.2.2` (DIST1), `3.3.3.3` (DIST2), `45.45.45.45` (BL1), `46.46.46.46` (BL2), `4.4.4.4` (HQ) — tous `FULL/ -`. [P-05]
- `show ip route 172.16.2.0` (Core) → **deux descripteurs**, `via 10.0.12.1` et `via 10.0.13.1`, métrique égale → **ECMP N-S prouvé**. [P-06]
- Sur **HQ-Router** — `show ip route ospf | include 172.16` → `O 172.16.2.0` et `O 172.16.3.0` via `10.0.1.1` → le DC a atteint l'edge. [P-07]
...
**Validation :** `http://172.16.2.11` = « Web-App 1 » [P-14], `http://172.16.2.12` = « Web-App 2 » [P-15], `http://172.16.2.10` = page LB [P-16].
...
**Validation :** `show interface status` → ports inutilisés `disabled` VLAN 998 ; ports routés `routed` ; ports serveurs `connected`. [P-09]
...
**Validation :**
- `show route` → `172.16.0.0/24 … 3 subnets` : le `/24` DMZ connecté + `S 172.16.2.0` + `S 172.16.3.0` via `192.168.200.2`. [P-20]
- Depuis **APP-WEB1** — `ping 8.8.8.8` → `TTL=249` (après 1–3 timeouts de build). [P-13]
- `show nat` → `DC-NET translate_hits ≠ 0` (session : `translate_hits = 3, untranslate_hits = 2`). **Ce compteur est la preuve durable de la sortie.** [P-12]
- `show xlate` montre l'entrée PAT `DC-NET` **seulement tant que le flux est vivant** (timeout ICMP 30 s). Ne pas lire une table vieillie comme un échec ; lire le compteur. [P-21]
...
|---|---|---|---|
| OSPF | `show ip ospf neighbor` | Spine ×4 / BL ×3 / Leaf ×2 / Core ×5, tous `FULL/ -`, pas de DR/BDR | [P-01], [P-03], [P-04], [P-05] |
| ECMP | `show ip route 172.16.2.0` (Core) | deux next-hops : `10.0.12.1` + `10.0.13.1` | [P-06] |
| Edge | `show ip route ospf \| include 172.16` (HQ) | `O 172.16.2.0` + `O 172.16.3.0` | [P-07] |
| Edge (ASA) | `show route` (ASA) | `3 subnets`, `S 172.16.2.0` + `S 172.16.3.0` | [P-20] |
| SVI | `show ip interface brief \| include Vlan21` | `172.16.2.1` / `172.16.3.1` `up/up` | [P-09] |
| E-O | APP-WEB1 `ping 172.16.3.10` | 4/4 (2e run) | [P-10] |
| N-S | PC campus `ping 172.16.2.11` + `tracert` | 4/4 ; chemin `DIST→Core→BL→Spine→Leaf1` | [P-11] |
| Sortie | APP-WEB1 `ping 8.8.8.8` + `show nat` | réponse ; `DC-NET translate_hits ≠ 0` | [P-13], [P-12] |
| Isolation | `show access-list DMZ-RESTRICT` (ASA) | ligne 5 `deny ip host 172.16.0.10 any` hitcnt monte sur WEB-PUBLIC→APP | [P-17] |
| Durcissement | `show interface status` | ports inutilisés `disabled` VLAN 998 | [P-09] |
...
| 3 | `show run interface Gi1/0/5` → `% Invalid input` | Commande lancée depuis `(config)#` | le marqueur `^` | `end` puis `show running-config interface …`, ou `do show …` — [P-18] |
| 4 | Ports d'accès campus flashent ambre → vert | Reconvergence STP cosmétique (TCN / jitter PT) — **pas** un PortFast manquant (`… fa0/3 detail` → « portfast mode ») | 0 % de perte partout ; `transitions to forwarding: 1` | Artefact PT, non combattu — [P-19] |
| 5 | Les 1–3 premiers paquets d'un flux N-S / sortie timeout | Build ARP + xlate/CEF sur plusieurs sauts | réponse au paquet 2–4 avec le bon TTL ; runs suivants 4/4 | Attendu — verdict lu sur les paquets suivants — [P-10] |
...
**[P-01] · Adjacences Spine** — DC-Spine1 `show ip ospf neighbor`, 4× `FULL/ -`

![Capture P4-24](../assets/captures/P4/Capture_P4_24.png)

**[P-03] · Adjacences Border Leaf + ECMP fabric** — DC-BorderLeaf2, 3 voisins, puis `route 172.16.2.0` via 10.0.9.1 + 10.0.11.1

![Capture P4-20](../assets/captures/P4/Capture_P4_20.png)

**[P-04] · Adjacences Leaf** — DC-Leaf1, 2× `FULL/ -`

![Capture P4-01](../assets/captures/P4/Capture_P4_01.png)

**[P-05] · Adjacences Core** — CORE-SW, 5 voisins (DIST1/2, BL1/2, HQ)

![Capture P4-23](../assets/captures/P4/Capture_P4_23.png)

**[P-06] · ECMP N-S au Core** — CORE-SW `route 172.16.2.0` via 10.0.12.1 + 10.0.13.1

![Capture P4-22](../assets/captures/P4/Capture_P4_22.png)

**[P-07] · Propagation vers l'edge** — HQ-Router `O 172.16.2.0` + `O 172.16.3.0`

![Capture P4-19](../assets/captures/P4/Capture_P4_19.png)

**[P-09] · SVI + durcissement** — DC-Leaf2 `Vlan220 up/up` + `interface status` (Gi1/0/5=220, reste 998)

![Capture P4-16](../assets/captures/P4/Capture_P4_16.png)

**[P-10] · Est-Ouest (+ timeouts de boot)** — PC campus ping `.2.1` / `.2.12` / `.3.10` (3 timeouts→réponse, puis 4/4)

![Capture P4-15](../assets/captures/P4/Capture_P4_15.png)

**[P-11] · Chemin Nord-Sud** — PC1 `tracert .2.11` via 10.0.3.10→10.0.12.1→10.0.9.10→10.0.4.2

![Capture P4-14](../assets/captures/P4/Capture_P4_14.png)

**[P-12] · Compteur de sortie** — ASA `show nat`, `DC-NET translate_hits=3, untranslate_hits=2`

![Capture P4-03](../assets/captures/P4/Capture_P4_03.png)

**[P-13] · Data-plane de sortie** — WEB-APP-1 `ping 8.8.8.8` réponse `TTL=249`

![Capture P4-04](../assets/captures/P4/Capture_P4_04.png)

**[P-14] · Backend App1** — `http://172.16.2.11` = « Web-App 1 »

![Capture P4-11](../assets/captures/P4/Capture_P4_11.png)

**[P-15] · Backend App2 (distinct)** — `http://172.16.2.12` = « Web-App 2 »

![Capture P4-10](../assets/captures/P4/Capture_P4_10.png)

**[P-16] · LB (auto-documente la dette)** — `http://172.16.2.10` = page LB

![Capture P4-09](../assets/captures/P4/Capture_P4_09.png)

**[P-17] · Isolation DMZ→APP** — ASA `DMZ-RESTRICT ligne 5 deny … hitcnt=4`

![Capture P4-05](../assets/captures/P4/Capture_P4_05.png)

**[P-18] · Incident 3 — piège config-mode** — `DC-LEAF2(config)# show run interface` → `% Invalid input` ×3

![Capture P4-07](../assets/captures/P4/Capture_P4_07.png)

**[P-19] · Incident 4 — ambre/PortFast** — ACC-SW1 `spanning-tree … fa0/3 detail` → « portfast mode », msg age 16

![Capture P4-06](../assets/captures/P4/Capture_P4_06.png)

**[P-20] · Joignabilité edge vers le DC** — ASA `show route`, `3 subnets`, `S 172.16.2.0` + `S 172.16.3.0`

![Capture P4-32](../assets/captures/P4/Capture_P4_32.png)

**[P-21] · Table NAT xlate (contexte)** — ASA `show xlate`, WEB-PUBLIC statique seul ; l'entrée dynamique `DC-NET` vieillit (sortie prouvée par le compteur [P-12])
